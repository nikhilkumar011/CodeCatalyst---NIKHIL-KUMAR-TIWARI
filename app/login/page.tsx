 ...
 * ------------------------------------------------------------
 * Signature element: a rotating wireframe orb made of connecting
 * nodes on the left panel — nodes lighting up like a handshake
 * between device and system, rather than a generic gradient blob.
 *
 * Palette: bg #050816 · primary #3B82F6 · secondary #8B5CF6 ·
 *          accent #22D3EE · success #10B981 · text #FFFFFF/#94A3B8
 *
 * next/image is imported per your template but unused below since
 * every visual here is vector (SVG/CSS) — swap in <Image /> if you
 * add a real product screenshot or team photo later.
 */
...
const Checkbox = ({
    ...
    <label htmlFor={id} className="flex items-center gap-2.5 cursor-pointer select-none group">
        <span className="relative flex items-center justify-center w-[18px] h-[18px] shrink-0">
            ...
            <span
                className="w-[18px] h-[18px] rounded-[5px] border transition-all duration-200
>>>>>>> 6f37f1cb4497cffea7ccf2d46de29b89cc4fc377
...
<<<<<<< HEAD
    ...
    <span className="text-[13px] text-[#94A3B8] group-hover:text-white/80 transition-colors">{label}</span>
  </label>
=======
            />
            <svg
    ...
    <div className="relative w-[min(70vw,420px)] h-[min(70vw,420px)] mx-auto">
      ...
      >
        ...
        {ACCESS_NODES.map(([x, y], i) => (
          <g key={i}>
            <circle cx={x} cy={y} r="4" fill="url(#nodeGlow)" opacity="0.6" />
            <motion.circle
              cx={x}
              cy={y}
              r={i === 13 ? 2.1 : 1.15}
          ...
          <div className="relative hidden lg:flex lg:w-1/2 flex-col items-center justify-center px-12 py-16 overflow-hidden">
            ...
            >
              ...
              <p className="mt-4 text-[#94A3B8] text-[15px] leading-relaxed">
                Briefly reads the paper, cites its claims, and turns dense
                research into flashcards and briefs you can actually study.
              </p>
            </motion.div>
          </div>
            ...
            >
              <div className="rounded-[23px] bg-[#0A0E1F]/80 backdrop-blur-2xl border border-white/[0.06] px-7 sm:px-9 py-9 sm:py-10 shadow-[0_8px_60px_-12px_rgba(59,130,246,0.25)]">
                ...
                <motion.div variants={fieldVariants}>
                  ...
                  <p className="mt-1.5 text-sm text-[#94A3B8]">Sign in to pick up right where you left off.</p>
                </motion.div>
                ...
                <form onSubmit={handleSubmit} noValidate className="mt-7 space-y-4">
                  ...
                  <motion.div variants={fieldVariants} className="flex items-center justify-between pt-1">
                    <Checkbox id="remember" checked={remember} onChange={() => setRemember((r) => !r)} label="Remember me" />
                  </motion.div>
...
<<<<<<< HEAD
                ...
                <motion.p variants={fieldVariants} className="mt-3 text-center text-[11px] text-[#5b6478] leading-relaxed">
                  By continuing, you agree to Briefly&apos;s{" "}
